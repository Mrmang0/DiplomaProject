<template>
  <v-dialog lazy v-model="dialog3" :full-width="true" max-width="500px">
    ...
    <v-card class="pa-4"> 
      ...
      <v-flex>
        <v-btn @click="login"
        color="primary">Війти</v-btn>
      </v-flex>
          </v-tab-item>
          <v-tab-item id="`tab-2`">
           <v-text-field
             label="Email"
             v-model="registerRequestData.email">
           </v-text-field>
            <v-text-field
             label="Прізвище"
             v-model="registerRequestData.patronym">
           </v-text-field>
            <v-text-field
             label="Ім'я"
             v-model="registerRequestData.name">
           </v-text-field>
            <v-text-field
             label="По-батькові"
             v-model="registerRequestData.surname">
           </v-text-field>
           <v-text-field
               name="input-10-1"
               label="Пароль"
              hint="Пароль повинен містити щонайменьше 8 символів"
              v-model="registerRequestData.password"
          min="8"
          :append-icon="e1 ? 'visibility' : 'visibility_off'"
          :append-icon-cb="() => (e1 = !e1)"
          :type="e1 ? 'password' : 'text'"
        ...
        <v-text-field
              ...
              hint="Пароль повинен містити щонайменьше 8 символів"
              v-model="registerRequestData.confirmPassword"
          min="8"
          :append-icon="e1 ? 'visibility' : 'visibility_off'"
          :append-icon-cb="() => (e1 = !e1)"
          :type="e1 ? 'password' : 'text'"
        ...
        color="primary">Зареєструватись</v-btn>
...
<script>
  export default {
    ...
    methods: {
      ...
      register(){
        console.log(this.registerRequestData)
      },
      clear () {
